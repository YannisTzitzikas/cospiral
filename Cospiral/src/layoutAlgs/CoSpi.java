...
/**
 * 
 * @author Yannis Tzitzikas (tzitzik@ics.forth.gr) : Algorithm idea and visualization methods.
 * @author Manos Chatzakis (chatzakis@ics.forth.gr): Labels, GUI application, SVG generator, CoSpi cleanup.
 * 
 * This class is the Java implementation of the CoSpi algorithm.
 * It contains input and visualization methods to load and run datasets to create layouts.
 * 
 * Dataset Format:
 * CSV or txt files with each value in a new line, and names seperated with commas.
 * See the example datasets folder of the project.
 * 
 * Last update: August 2020, Merged files and code cleanup.
 * 
 * BUG: Labels wont work properly for pie charts for non value-sorted datasets. Sort them first.
 *
 */
public class CoSpi {
	...
	public static int K; //Number of entities/values to be visualized
	...
	public static ArrayList<Rectangle>rects              = new ArrayList<>(); //Model rectangle list (to be used by the compact algorithm)
	...
	public static ArrayList<Integer>radiiWhenScaleofNumberOfVisObjectChanges = new ArrayList<>(); //Needed for the axisX
	 ...
	 */
	public static void loadDataAndRun(String filename, int valCol,int labCol, int min, int max,
			VisConfig conf,boolean usingGUI,boolean createSVG){
		   
		String csvFile = filename;		  
		CSVReader.separateMapToLists(originalValues, originalNames,CSVReader.readValuesAndCreateMap(valCol, labCol, CSVReader.readFile(csvFile)));
	 ...
	 * @param createSVG
	 */
	public static void loadDataAndRunPieChart(String filename, int groupByCol, int valCol,int nameCol,
		  int min, int max, VisConfig conf,boolean usingGUI,boolean createSVG) {
	 ...
	 * @param visConf
	 */
	public static void loadDataFromFilesAndCompare(String[] filenames, int[] columns, int NormalizeMin, int NormalizeMax, VisConfig visConf,boolean usingGUI,boolean createSVG) {
	
		// Part A: Loads the  data from all files and stores them in a map
		Map<String,ArrayList<String>> mapGV = new TreeMap<>(); 
	    for (int i=0; i<filenames.length; i++) {
	    	 ArrayList<String> dataFromFile  = CSVReader.getValueColumn(CSVReader.readFile(filenames[i]),columns[i]); // reading the data from one file
	    	 mapGV.put(filenames[i]+i, dataFromFile); // put them in the map. The "+i" is for the case we use the same filename more than once
	      }
	    // Part B: It calls the method that will produce the pie chart
	    createPieChart(mapGV,null,NormalizeMin,  NormalizeMax, visConf,usingGUI,createSVG); // calls the piechart visualizer
	 ...
	 */
	public static void  loadDataFromSingleFileAndCompareFrequencies(String filename, int[] columns, int NormalizeMin, int NormalizeMax, VisConfig visConf,boolean usingGUI,boolean createSVG) {
		...
		loadDataFromFilesAndCompareFrequencies(filenames,columns, NormalizeMin, NormalizeMax,  visConf,usingGUI,createSVG) ;
	}
	...
	public static void  loadDataFromFilesAndCompareFrequencies(String[] filenames, int[] columns, int NormalizeMin, int NormalizeMax, VisConfig visConf, boolean usingGUI, boolean createSVG) {
	    ...
	    for (int i=0; i<filenames.length; i++) {
	    	 ...
	    	 // A. Find the distinct values and their frequencies and to then keep the frequencies
	    	 ...
	    	 mapGV.put(filenames[i]+i, freqsOnly); // put them in the map. The "+i" is for the case we use the same filename more than once
	      }
	    // Part B: It calls the method that will produce the pie chart
	...
	public static void createPieChart(Map<String,ArrayList<String>> mapGV, Map<String,ArrayList<String>> mapNames,
		 int NormalizeMin, int NormalizeMax, VisConfig conf, boolean usingGUI, boolean createSVG) {
		  ...
		  // Part A:  General Setup for Angles  (the angle of the entire diagram, the angle gap between the slices)
	      ...
	      if(mapNames == null) keepNames = false;
	      
	      // Part B: The angles for each pie (hereafter with pie I mean slice)
	      int pieNum = mapGV.keySet().size();  System.out.println("Number of pies: " + pieNum);
		  ...
		  //Part C:  Conversion of the Map to .. Integer, for finding also the Min Max which are needed for proper normalization
		  ...
		  for (String key: mapGV.keySet()) {
			  ArrayList<Integer> valsInt = CSVReader.toIntDescending(mapGV.get(key)); // String to Integer and sorting desc
			  ArrayList<String>valsNames = null;
			  if(keepNames) valsNames	  = CSVReader.getNamesOfBuildings(mapNames.get(key));
			  mapGVint.put(key, valsInt); // addition to the map
			  if(keepNames) mapGVNames.put(key, valsNames);
			  
			  if (valsInt.get(0) >  allValuesMax)  allValuesMax = valsInt.get(0);  // update allValuesMax if required
			  if (valsInt.get(valsInt.size()-1) <  allValuesMin)  allValuesMin = valsInt.get(valsInt.size()-1);  // update allValuesMin if required
		  }
		  ...
		  Color[] sliceColors = {Color.red,Color.yellow,Color.green,Color.pink,new Color(255,255,153),  // yellow-like
                  Color.cyan,Color.LIGHT_GRAY,Color.magenta,Color.orange,
                  new Color(255,229,204),// mpez
		  ...
		  for (String key: mapGV.keySet()) {  // for each grouByVal, i.e. for each pie
			  ...
			  if(keepNames)names = mapGVNames.get(key);
			  ...
			  if(keepNames) originalNames.addAll(names);
	...
	/**
	 * This method runs the Concentric Spiral algorithm and manipulates the dataset visualization using the further parameters. 
	 * @param values The normalized rectangle side values.
	 * @param conf The visual configurations.
	 * @param incremental Re-initialize the pixels at the start (used mostly to create pie charts).
	 * @param lastCall This parameter is set true if the method is called once or for pie charts to declare that the further parameters should be added or not.
	 ...
	 */
	public static void visualize(ArrayList<Integer>values,VisConfig conf,
			boolean incremental,boolean lastCall,boolean GUIMode,
		...
		if((originalValues==null)||(originalValues.isEmpty())) {originalValues = new ArrayList<>(values);}
		
		copyInfoToRects(values);
		radiiWhenScaleofNumberOfVisObjectChanges.clear();
		radiiWhenScaleOfValuesChanges.clear();	
		
		if (!incremental) {pic.preparePixels(pixels, N, Color.white);}		
		
		if(!pieChart) {defColor = conf.getRectColor();}
		
		ConcentricSpiralCompact(conf.getDirection(), 
				  conf.getExpandStyle(),
				  conf.getDrawStyle(),
				  conf.getRoadSize(),
				  conf.getAngleMin(),
				  conf.getAngleMax(),
				  conf.getAxes(),
				  conf.getAllowedOverlap());
		
		if (conf.getAxes()!=Axes.NoAxes) {MathUtils.visualizeAxes(values, conf, radiiWhenScaleOfValuesChanges, radiiWhenScaleofNumberOfVisObjectChanges, pixels, N);}
	 ...
	 * @param allowedOverlap
	 */
	public static void ConcentricSpiralCompact(Direction directionMode, ExpandStyle expandStyle,  DrawStyle drawStyle, int roadSz, double AngleMin, double AngleMax, Axes axes, boolean allowedOverlap) {
		...
		while ((curBuildingIndex<K) && (!outOfPaper)) {  // while there are buildings to draw
			...
			minChordIncrement = rects.get(curBuildingIndex).len;  // the side of the  square to be drawn (for placing it just *above* the x-axis)
			
			if (minChordIncrement==0) { // (Jan 18, 2020) if the building to be visualized has zero size  (this is needed for the case where filled rings are used for the too small values)
	 			System.out.println("ZERO SIZE SHAPE");
	 			int radiusForTheFilledRing = rad + maxSqSizeInRing+roadSize; // the radius from which the filled ring should start
			...
			if (curBuildingIndex>0) {  // i.e. if not the first building
				...
				//minChordIncrement= minChordIncrement + (int) ((curSquareSize+rects.get(curBuildingIndex).len)/Math.sqrt(2));
			}
			
			angleIncrement = MathUtils.degreesOfChord(minChordIncrement, rad); // degrees corresponding to the chord with size  minChordIncrement
			curAngle = curAngle+angleIncrement;
		 	...
		 	if (curAngle >= AngleMax) { // if true then a new ring should be started	 
		    	...
		    	angleIncrement = MathUtils.degreesOfChord(minChordIncrement, rad); // for placing the first square *above* the x-axis
			...
			if (MathUtils.isEmpty(x,y,curRect.len,pixels,N) || allowedOverlap) {  // not needed for normal spiral
				...
				if (axes!=Axes.NoAxes) keepInfoForAxes(curBuildingIndex-1,curRect.len,rad);
			}
			else {
	...
	public static void fillAsARing(int curIndex, int curRadius) {
		  ...
		  //int radiusOuter =  (int) Math.round( Math.sqrt(curRadius*curRadius + ( sumOfOriginalValues / Math.PI))); // not normalized
	...
	public static void copyInfoToRects(ArrayList<Integer>values) { 
		...
		/*	  if(originalValues.size() != values.size() || originalNames.size() != values.size()
					  || originalValues.isEmpty() || originalNames.isEmpty()) {				  
				  rects.add(new Rectangle(0,0,values.get(i),-1,"NoName",defColor));
			  }
			  else{
				  rects.add(new Rectangle(0,0,values.get(i),originalValues.get(i),originalNames.get(i),defColor));
			  }*/
		  ...
		  if (vallog-vallogInt<0.03) { // val  is a power of 10 (todo: 2 check precision issues)
			  if (vallog>=1) { // for not drawing an axis for 1
				  if (!radiiWhenScaleOfValuesChanges.contains(radius)) { // if that radius is not already in the arraylist
				      ...
				      //System.out.println(">>>Original val:"+originalValues.get(curIndex) + " Log val:"+vallog);
				  }
			  }
		  }
	  }
	...
	static void keepInfoForRects(int x,int y, int len, int rank) {
		...
		if(curIndex>=originalValues.size() || curIndex>=originalNames.size()){rectangles.add(new Rectangle(x, y, len, 0,"-", defColor));}
		else{rectangles.add(new Rectangle(x, y, len, originalValues.get(curIndex),rank,originalNames.get(curIndex), defColor));}
	}
	...
	public static void main(String [] args) {
		...
		CoSpi.loadDataAndRun("Resources/cvAu_cut.csv", 1, 0,10, 80, conf,false,false);
		...
		///Map<String,ArrayList<String>> mapGV =  CSVReader.groupValsAndNames("Resources/companies.csv",1,2,0);
		//System.out.println(mapGV);
		
		//loadDataFromFileAndVisualizePie("Resources/citiesContinents.csv", 1, 2, 10, 40, conf); 
		
		//loadDataAndRunPieChart("Resources/companies.csv",1,2,0,10,40,conf);
		
		/*ArrayList vals = 
				  SyntheticDataGenerator
				  	.DecreasingByPercentage(15, 100, 8, 2); // size, maxsize, minsize, decreasePercentage
		  			//.DecreasingByPercentage(1*1000*1000, 1, 1, 2); // size, maxsize, minsize, decreasePercentage (to exw paei ews 15 millions)
		  originalValues = new ArrayList<>(vals); // Jan 5, 2020
		  visualize(vals, conf, false,true, false, false, false);
		*/
	...
	public static void  clusteredLODUniform() {
		  pic.preparePixels(pixels,N,Color.white);
		  ArrayList vals = 	  SyntheticDataGenerator
		  ...
		  double anglePerDomain =  (piNum*Math.PI - angleGap) / DNum;  // h katharh gonia pou prepei na exei kathe ena domain
		  double angleGapAfterADomain = angleGap / DNum;

		  double fromAngle = 0;
		  for (int i=0; i < DNum; i++) {
			  switch (i) {
			  	case 0: defColor= Color.black;		  	break;
			  	case 1: defColor= Color.blue;		  	break;
			  	case 2: defColor= Color.red;		  	break;
			  	case 3: defColor= Color.green;		  	break;
			  	case 4: defColor= Color.yellow;		  	break;
			  	case 5: defColor= Color.cyan;		  	break;
			  	case 6: defColor= Color.lightGray;		break;
			  	case 7: defColor= Color.magenta;		break;
			  	case 8: defColor= Color.orange;		  	break;
			  }
			  ConcentricSpiralCompact(Direction.Expand, ExpandStyle.Spiral, DrawStyle.Filled, 5, fromAngle, fromAngle+anglePerDomain, Axes.NoAxes,false);   
	...
	public static void  clusteredLOD() {
		  ...
		  // for not uniform:
		  int Kbefore = K; // Ã¯Â¿Â½Ã¯Â¿Â½ Ã¯Â¿Â½Ã¯Â¿Â½Ã¯Â¿Â½Ã¯Â¿Â½Ã¯Â¿Â½Ã¯Â¿Â½ Ã¯Â¿Â½Ã¯Â¿Â½Ã¯Â¿Â½ Ã¯Â¿Â½Ã¯Â¿Â½Ã¯Â¿Â½Ã¯Â¿Â½Ã¯Â¿Â½Ã¯Â¿Â½Ã¯Â¿Â½
		  int Kfake = 10;
		  ...
		  for (int i=0; i < DNum; i++) {
			  ...
			  switch (i) {
			  	case 0: defColor= Color.red;		  	break;
			  	case 1: defColor= Color.blue;		  	break;
			  	case 2: defColor= Color.pink;		  	break;
			  	case 3: defColor= Color.green;		  	break;
			  	case 4: defColor= Color.DARK_GRAY;		break;
			  	case 5: defColor= Color.cyan;		  	break;
			  	case 6: defColor= Color.lightGray;		break;
			  	case 7: defColor= Color.magenta;		break;
			  	case 8: defColor= Color.orange;		  	break;
			  ...
			  ConcentricSpiralCompact(Direction.Expand, ExpandStyle.Spiral, DrawStyle.Filled, 5, fromAngle, fromAngle+anglePerDomain, Axes.NoAxes, false);   // This is the stable version
			  fromAngle = fromAngle+anglePerDomain + angleGapAfterADomain;
			  K = multiplier* K;
