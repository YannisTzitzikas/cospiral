...
public class MathUtils {
	...
	/**
	 * Returns whether or not the coordinates (x,y) of the pixels are occupied by another rectangle
	...
	public static boolean isEmpty(int x, int y, int len,Pixel [][] pixels,int N) {
		...
		return 		(
					(pixels[x][y].getColor() == Color.white && !pixels[x][y].isOccupied())     // center
					&&
					(pixels[xleft][ydown].getColor() == Color.white && !pixels[xleft][ydown].isOccupied()) // bottom left corner
					&& 
					(pixels[xright][ydown].getColor() == Color.white && !pixels[xright][ydown].isOccupied()) // bottom right corner
					&&
					(pixels[xleft][yup].getColor() == Color.white && !pixels[xleft][yup].isOccupied()) // up left corner
					&& 
					(pixels[xright][yup].getColor() == Color.white && !pixels[xright][yup].isOccupied()) // up right corner
					);	
	}
	
	/**
	 * Draws the axes circles based on the data hold when the rectangles of the layout are drawn.
	 * @param vals The values of the dataset visualized
	 * @param conf The visual configurations
	 * @param radiiWhenScaleOfValuesChanges The data for the rectangles of which the value change (y axis)
	 * @param radiiWhenScaleofNumberOfVisObjectChanges The data for when the number of the rectangles to be visualized changes (x axis) 
	 * @param pixels The pixels of the layout
	...
	public static void visualizeAxes(ArrayList vals, VisConfig conf,ArrayList<Integer> radiiWhenScaleOfValuesChanges,ArrayList<Integer> radiiWhenScaleofNumberOfVisObjectChanges,Pixel pixels[][],int N) {
		  ...
		  if ((conf.getAxes()==Axes.AxisX) || (conf.getAxes()==Axes.AxesXY)) { // if X axis is required
			  ...
			  for (int radNumObjScal: radiiWhenScaleofNumberOfVisObjectChanges) {
				  //auxiliaryCircle(radNumObjScal, Color.GRAY,scale++); // it increases the scale for increasing the thickness
				  auxiliaryCircle(radNumObjScal, Color.GRAY,scale++,conf.getAngleMin(),conf.getAngleMax(),pixels,N); // it increases the scale for increasing the thickness
			...
			for (double m=angleMin; m<=angleMax; m+= 0.001) { // loop for the cycle: from angleMin to angleMax
					...
					if (isInCanvas(x,y,N)) {
						...
						for (int j=-paxos; j<=paxos; j++) { // for increasing the density
							if (isInCanvas(x,y+j,N))	pixels[x][y+j].setColor(color) ; 
							if (isInCanvas(x+j,y,N))  pixels[x+j][y].setColor(color);
						}
					} else {
	...
	/**
	 * Fills the layout with a ring (made of multiple auxiliary circles). This method is used to visualize values with normilized value 0.
