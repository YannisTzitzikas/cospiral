...
public class Labels {
    ...
    /**
     * Adds labels to the rectangles of the layout based on the visual
     * configurations.
     *
     * @param conf
     * @param rectangles
    ...
    public static void addLabels(VisConfig conf, ArrayList<Rectangle> rectangles, Picture pic, int N) {
        ...
        if (conf.getLabelDecreasingRate() <= 0) {
            return;
        }
        ...
        for (int i = 0; i < rectangles.size(); i++) {
            ...
            String value = rectangles.get(i).getRealValueAsStr();
            ...
            int y = N - rectangles.get(i).getY() - height / 2 + metrics.getAscent();
            int yDown = N - rectangles.get(i).getY() - height / 2 + diffRate + metrics.getAscent();
            int yUp = N - rectangles.get(i).getY() - height / 2 - diffRate + metrics.getAscent();

            // Getting how many labels the layout includes (to re-calculate the y
            ...
            case twoLabels:
                ...
                diffRate = height / 2;
                yDown = N - rectangles.get(i).getY() - height / 2 + diffRate + metrics.getAscent();
                yUp = N - rectangles.get(i).getY() - height / 2 - diffRate + metrics.getAscent();
                if (conf.isShowRank()) {
                    g.drawString(rank, xRank, yUp);
                    if (conf.isShowVal()) {
                        g.drawString(value, xValue, yDown);
                    } else {
                        g.drawString(name, xName, yDown);
                    }
                } else {
    ...
    public static void writeInfoToImage(Image pic, int N, int size, Color color, int MAX_SIZE, int MIN_SIZE,
        ...
        int height = metrics.getHeight();
        int maxWidth = Math.max(g.getFontMetrics().stringWidth(values),
                (Math.max(g.getFontMetrics().stringWidth(nMax), g.getFontMetrics().stringWidth(nRmax))));
    ...
    /**
     * Add information to visualizations created by SyntheticDataGenerator
     *
     * @param pic
     * @param N
     * @param size
